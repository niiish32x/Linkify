...
import Linkify.shortlink.project.dao.entity.*;
import Linkify.shortlink.project.dao.mapper.*;
...
import Linkify.shortlink.project.common.config.GotoDomainWhiteListConfiguration;
import Linkify.shortlink.project.common.convention.exception.ClientException;
import Linkify.shortlink.project.common.convention.exception.ServiceException;
import Linkify.shortlink.project.common.enums.VailDateTypeEnum;
...
import Linkify.shortlink.project.mq.producer.DelayShortLinkStatsProducer;
import Linkify.shortlink.project.service.LinkStatsTodayService;
import Linkify.shortlink.project.service.ShortLinkService;
import Linkify.shortlink.project.toolkit.HashUtil;
import Linkify.shortlink.project.toolkit.LinkUtil;
...
import static Linkify.shortlink.project.common.constant.RedisKeyConstant.*;
import static Linkify.shortlink.project.common.constant.RedisKeyConstant.LOCK_GID_UPDATE_KEY;
import static Linkify.shortlink.project.common.constant.ShortLinkConstant.AMAP_REMOTE_URL;
...
public class ShortLinkServiceImpl extends ServiceImpl<ShortLinkMapper, ShortLinkDO> implements ShortLinkService {
    ...
    public ShortLinkCreateRespDTO createShortLink(ShortLinkCreateReqDTO requestParam) {
        ...
        // ! 缓存预热 将新创建的短链接 直接加入Redis中 防止后续的访问过多扫盘访问数据库
    ...
    public void updateShortLink(ShortLinkUpdateReqDTO requestParam) {
        ...
        verificationWhitelist(requestParam.getOriginUrl());
        // 1. 从入参读入需要修改的短链接 g
        // 2. 根据 原始Gid getOriginGid 以及 完整的长连接 getFullShortUrl 进行查找
        LambdaQueryWrapper<ShortLinkDO> queryWrapper = Wrappers.lambdaQuery(ShortLinkDO.class)
        ...
        // * 修改 分为两种情况:
        // * 1. 不修改链接分组: 这种就比较简单直接对该链接进行修改即可
        // * 2. 修改链接分组: 那么就要同步更新 链接分组下的信息(如该链接下存在哪些链接 链接数量) 在修改该链接的同时就要对要分组信息进行修改 因此需要上读写锁
        if (Objects.equals(hasShortLinkDO.getGid(), requestParam.getGid())) {
            ...
            * 读写锁:
            * 读写锁用于对共享资源的并发控制
            * 读写锁的特点是：同一时刻允许多个线程对共享资源进行读操作；
            * 同一时刻只允许一个线程对共享资源进行写操作；当进行写操作时，同一时刻其他线程的读操作会被阻塞；
    ...
    public void shortLinkStats(String fullShortUrl, String gid, ShortLinkStatsRecordDTO statsRecord) {
        /*
        * Java中的Optional类是用于处理可能为空（null）的值的容器类。
        * 它的主要作用是解决空指针异常（NullPointerException）的问题，
        * 并提供了一种更加优雅和安全的方式来处理可能为空的值。
        *
        * ofNullable(T value)：创建一个Optional实例，可以包含一个可能为空的值。
        * isPresent()：判断Optional实例是否包含非空值。
        * get()：获取Optional实例中的值，如果值为空，则抛出NoSuchElementException异常。
        * orElse(T other)：获取Optional实例中的值，如果值为空，则返回指定的默认值。
        * ifPresent(Consumer<? super T> consumer)：如果Optional实例中的值非空，则执行指定的操作。
        *
        * */
        ...
        // * 尝试获取读锁
        // * 如果正在读 那么说明该条链接正在被访问 那么延迟监控 将请求先装入延迟队列中
        if (!rLock.tryLock()) {
        ...
        * */
        try {
            if (StrUtil.isBlank(gid)) {
                // * 分组ID 为空 那么要找到分组ID GID -> 对应的是在进行链接跳转时 原始链接非空的情况 即存在原始链接可以进行正常跳转
                // * 所以不需要传入 新的GID 直接根据 FullShortUrl 找到相应GID 进行相关监控操作即可
                LambdaQueryWrapper<ShortLinkGotoDO> queryWrapper = Wrappers.lambdaQuery(ShortLinkGotoDO.class)
                        .eq(ShortLinkGotoDO::getFullShortUrl, fullShortUrl);
                ShortLinkGotoDO shortLinkGotoDO = shortLinkGotoMapper.selectOne(queryWrapper);
                gid = shortLinkGotoDO.getGid();
            }
            // * 分组非空 其实就是跳转时 已经到找到了 数据库中完整长链接的那一步了 那么既然已经到了查数据库这一步 那么直接顺便返回gid 相当于是操作进行了优化
